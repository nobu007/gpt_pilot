...
from utils.function_calling import FunctionType, parse_agent_response
from utils.llm_connection import (
...
class TestRetryOnException:
    def setup_method(self):
        self.function: FunctionType = {
            ...
            "parameters": {
                "type": "object",
                "properties": {"foo": {"type": "string"}, "boolean": {"type": "boolean"}, "items": {"type": "array"}},
                "required": ["foo"],
            },
        }
    ...
    def test_incomplete_json_item(self, mock_styled_text):
        # Given incomplete JSON
        wrapper, args = self._create_wrapped_function(['{"foo": "bar",', ' "boolean"', ": true}"])
    ...
    def test_incomplete_json_array(self, mock_styled_text):
        # Given incomplete JSON
        wrapper, args = self._create_wrapped_function(['{"foo": "bar", "items": [1, 2, 3, "4"', ", 5]}"])
    ...
    def test_rate_limit_error(self, mock_sleep, mock_post, monkeypatch):
        ...
        mock_responses = [
            Mock(
                ...
                text="""{
            "error": {
                "message": "Rate limit reached for 10KTPM-200RPM in organization org-OASFC7k1Ff5IzueeLArhQtnT on tokens per min. Limit: 10000 / min. """
                + error_text
                + """",
                "type": "tokens",
                "param": null,
        ...
        content = "DONE"
        success_text = (
            '{"id": "gen-123", "choices": [{"index": 0, "delta": {"role": "assistant", "content": "'
            + content
            + '"}}]}'
        )
        ...
        data = {"model": "gpt-4", "messages": [{"role": "user", "content": "testing"}]}
