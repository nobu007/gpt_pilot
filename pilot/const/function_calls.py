        ...
        "parameters": {
            ...
            "properties": {
                f"{return_var_name}": {
                    ...
                    "items": {"type": "string", "description": f"{name_singular}"},
                },
            },
...
def step_command_definition(extended=False):
    ...
    properties = {
        "type": {"const": "command", "description": dev_step_type_description()},
...
def step_save_file_definition():
    return {
        "type": "object",
        "properties": {
            "type": {"const": "save_file", "description": dev_step_type_description()},
            "save_file": {
                "type": "object",
                "description": "A file that should be created or updated.",
                "properties": {
                    "path": {"type": "string", "description": "Full path of the file with the file name."},
                    "code_change_description": {"type": "string", "description": "Empty string"},
                },
...
def command_definition(
    description_command="A single command that needs to be executed.",
    description_timeout="Timeout in milliseconds that represent the approximate time this command takes to finish. "
...
USER_STORIES = {
    "definitions": [return_array_from_prompt("user stories", "user story", "stories")],
    "functions": {"process_user_stories": process_user_stories},
}

USER_TASKS = {
    "definitions": [return_array_from_prompt("user tasks", "user task", "tasks")],
    "functions": {"process_user_tasks": process_user_tasks},
}
...
ARCHITECTURE = {
    ...
    "functions": {"process_technologies": lambda technologies: technologies},
}
    ...
    "definitions": [
        {
            ...
            "parameters": command_definition(
                "Command that starts the app. If app can't be started for some reason, return command as empty string ''."
            ),
        },
    ],
    "functions": {"process_commands": run_commands},
}
...
IMPLEMENT_TASK = {
    ...
    "functions": {"parse_development_task": lambda tasks: tasks},
}
...
DEVELOPMENT_PLAN = {
    ...
    "functions": {"implement_development_plan": lambda plan: plan},
}
...
EXECUTE_COMMANDS = {
    ...
    "functions": {"execute_commands": lambda commands: commands},
}
...
GET_FILE_TO_MODIFY = {
    ...
    "functions": {"get_file_to_modify": lambda file: file},
}
    ...
    "definitions": [
        {
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    "command": command_definition(
                        "Command that needs to be run to test the changes.",
                        "Timeout in milliseconds that represent the approximate time this command takes to finish. If you need to run a command that doesn't finish by itself (eg. a command to run an app), put the timeout to 3000 milliseconds. If you need to create a directory that doesn't exist and is not the root project directory, always create it by running a command `mkdir`",
                    ),
...
DEBUG_STEPS_BREAKDOWN = {
    ...
    "functions": {"start_debugging": lambda steps: steps},
}
