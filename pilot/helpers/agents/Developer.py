...
from const.messages import AFFIRMATIVE_ANSWERS, NEGATIVE_ANSWERS, NONE_OF_THESE, STUCK_IN_LOOP, WHEN_USER_DONE
from database.database import (
    edit_development_plan,
    edit_feature_plan,
    get_progress_steps,
    save_progress,
    update_app_status,
...
from helpers.cli import execute_command_and_check_cli_response, run_command_until_success
from helpers.Debugger import Debugger
from helpers.exceptions import TokenLimitError, TooDeepRecursionError
from logger.logger import logger
from prompts.prompts import ask_user
from utils.exit import trace_code_event
from utils.print import print_step_progress, print_task_progress
...
from utils.utils import array_of_objects_to_string, generate_app_data, should_execute_step, step_already_finished
...
class Developer(Agent):
        ...
        while len(finished_tasks) < len(self.project.development_plan):
            ...
            if self.project.tasks_to_load:
                ...
                # if it is last task to load, execute it to check if it's finished
                    ...
                    readme_dev_step = next(
                        (
                            el
                            for el in self.project.dev_steps_to_load
                            if "create_readme.prompt" in el.get("prompt_path", "")
                        ),
                        None,
                    )
    ...
    def implement_task(self, i, task_source, development_task=None):
        ...
        print(
            color_green_bold(f"Implementing task #{i + 1}: ") + color_green(f' {development_task["description"]}\n'),
            category="pythagora",
        )
        ...
        if self.project.dev_steps_to_load and "breakdown.prompt" in self.project.dev_steps_to_load[0]["prompt_path"]:
            ...
            instructions = convo_dev_task.send_message(
        ...
        if self.project.dev_steps_to_load and "parse_task.prompt" in self.project.dev_steps_to_load[0]["prompt_path"]:
            ...
            response = convo_dev_task.send_message(
            ...
            last_parse_task_id = (
                int(self.project.checkpoints["last_development_step"]["id"])
                if self.project.checkpoints["last_development_step"] is not None
                else None
            )
        ...
        if self.project.dev_steps_to_load:
            ...
            self.project.last_iteration = next(
                (
                    el
                    for el in reversed(self.project.dev_steps_to_load)
                    if "iteration.prompt" in el.get("prompt_path", "")
                ),
                None,
            )
            ...
            self.project.last_detailed_user_review_goal = next(
                (
                    el
                    for el in reversed(self.project.dev_steps_to_load)
                    if "define_user_review_goal.prompt" in el.get("prompt_path", "")
                ),
                None,
            )
            ...
            if self.project.last_iteration is None and self.project.last_detailed_user_review_goal is None:
                ...
                if self.run_command is None:
                    self.run_command = next(
                        (
                            el
                            for el in reversed(self.project.dev_steps_to_load)
                            if "get_run_command.prompt" in el.get("prompt_path", "")
                        ),
                        None,
                    )
                ...
                ids = [
                    ...
                    self.project.last_detailed_user_review_goal["id"]
                    if self.project.last_detailed_user_review_goal
                    else None,
                ]
    ...
    def edit_task(self, task_source, task):
        ...
        if response.lower() in NEGATIVE_ANSWERS + ["skip task"]:
            ...
            self.project.development_plan = [
                element for element in self.project.development_plan if element["description"] != task["description"]
            ]
            if task_source == "app":
                db_task_skip = edit_development_plan(
                    self.project.args["app_id"], {"development_plan": self.project.development_plan}
                )
            else:
                db_task_skip = edit_feature_plan(
                    self.project.args["app_id"],
                    {"llm_response": {"text": json.dumps({"plan": self.project.development_plan})}},
                )
            ...
            if task_source == "app":
                db_task_edit = edit_development_plan(
                    self.project.args["app_id"], {"development_plan": self.project.development_plan}
                )
            else:
                db_task_edit = edit_feature_plan(
                    self.project.args["app_id"],
                    {"llm_response": {"text": json.dumps({"plan": self.project.development_plan})}},
                )
    ...
    def step_human_intervention(self, convo, task_steps: list, step_index):
        ...
        while True:
            ...
            if self.run_command:
                if self.project.check_ipc():
                    ...
                    human_intervention_description += color_yellow_bold(
                        '\n\nIf you want to run the app, just type "r" and press ENTER and that will run `'
                        + self.run_command
                        + "`"
                    )
            ...
            response = self.project.ask_for_human_intervention(
                ...
                human_intervention_description,
                cbs={
                    "r": lambda conv: run_command_until_success(
                        conv,
                        self.run_command,
            ...
            if response["user_input"] == "continue":
                ...
                response["success"] = self.debugger.debug(
                    convo,
                    user_input=response["user_input"],
                    issue_description=step["human_intervention_description"]
                    if "human_intervention_description" in step
                    else "",
                    task_steps=task_steps,
                    step_index=step_index,
                )
    ...
    def step_test(self, convo, test_command, task_steps=None, step_index=None):
        ...
        if should_rerun_command == "NO":
            ...
            cli_response, llm_response = execute_command_and_check_cli_response(
                convo, test_command, task_steps=task_steps, step_index=step_index
            )
            ...
            result = {"success": llm_response in ["DONE", "SKIP"], "cli_response": cli_response}
    ...
    def task_postprocessing(
        self, convo, development_task, continue_development, task_result, last_branch_name, task_source
    ):
        if self.project.last_detailed_user_review_goal is None:
            ...
            if development_task is not None:
                ...
                detailed_user_review_goal = convo.send_message(
                    "development/define_user_review_goal.prompt", {"os": platform.system()}, should_log_message=False
                )
                convo.remove_last_x_messages(2)
        else:
    ...
    def should_retry_step_implementation(self, step, step_implementation_try):
        ...
        while answer is None or answer.lower() not in AFFIRMATIVE_ANSWERS:
            ...
            answer = styled_text(self.project, "Type y/n")
    ...
    def execute_task(
        ...
        for i, step in enumerate(task_steps):
            ...
            if (
                step["type"] in ["save_file", "code_change", "modify_file"]
                and "path" in step[step["type"]]
                and step[step["type"]]["path"] not in self.modified_files
            ):
            ...
            while True:
                try:
                    ...
                    if step["type"] == "command":
                        result = self.step_command_run(
                            convo, task_steps, i, success_with_cli_response=need_to_see_output
                        )
        ...
        convo.load_branch(function_uuid)
        return self.task_postprocessing(
            convo, development_task, continue_development, result, function_uuid, task_source
        )

    def continue_development(self, iteration_convo, last_branch_name, continue_description="", development_task=None):
        llm_solutions = (
            self.project.last_iteration["prompt_data"]["previous_solutions"]
            if (self.project.last_iteration and "previous_solutions" in self.project.last_iteration["prompt_data"])
            else []
        )
        alternative_solutions_to_current_issue = (
            self.project.last_iteration["prompt_data"]["alternative_solutions_to_current_issue"]
            if (
                self.project.last_iteration
                and "alternative_solutions_to_current_issue" in self.project.last_iteration["prompt_data"]
            )
            else []
        )
        tried_alternative_solutions_to_current_issue = (
            self.project.last_iteration["prompt_data"]["tried_alternative_solutions_to_current_issue"]
            if (
                self.project.last_iteration
                and "tried_alternative_solutions_to_current_issue" in self.project.last_iteration["prompt_data"]
            )
            else []
        )
        next_solution_to_try = None
        iteration_count = (
            self.project.last_iteration["prompt_data"]["iteration_count"]
            if (self.project.last_iteration and "iteration_count" in self.project.last_iteration["prompt_data"])
            else 0
        )
        while True:
            ...
            user_description = (
                (
                    "Here is a description of what should be working: \n\n"
            ...
            response = self.project.ask_for_human_intervention(
                user_description,
                cbs={
                    "r": lambda convo: run_command_until_success(
                        convo,
                        self.run_command,
                        # name the process so the LLM can kill it
                        command_id="app",
            ...
            if user_feedback is not None:
                ...
                if stuck_in_loop:
                    ...
                    if len(alternative_solutions_to_current_issue) > 0:
                        next_solution_to_try_index = self.ask_user_for_next_solution(
                            alternative_solutions_to_current_issue
                        )
                        if next_solution_to_try_index == NONE_OF_THESE:
                            next_solution_to_try = self.get_alternative_solutions(
                                development_task,
                                user_feedback,
                                llm_solutions,
                                tried_alternative_solutions_to_current_issue + alternative_solutions_to_current_issue,
                            )
                        else:
                            next_solution_to_try = alternative_solutions_to_current_issue.pop(
                        ...
                        ) = self.get_alternative_solutions(
                            development_task,
                            user_feedback,
                            llm_solutions,
                            tried_alternative_solutions_to_current_issue,
                ...
                print_task_progress(
                    1, 1, development_task["description"], "troubleshooting", "in_progress", len(llm_solutions) + 1
                )
                iteration_convo = AgentConvo(self)
                iteration_description = iteration_convo.send_message(
                    "development/iteration.prompt",
                    {
                        "name": self.project.args["name"],
                        "app_type": self.project.args["app_type"],
                        "app_summary": self.project.project_description,
                ...
                llm_solutions.append({"user_feedback": user_feedback, "llm_proposal": iteration_description})

                instructions_prefix = " ".join(iteration_description.split()[:5])
                instructions_postfix = " ".join(iteration_description.split()[-5:])

                llm_response = iteration_convo.send_message(
                    "development/parse_task.prompt",
                    {
                        "os": platform.system(),
                        "instructions_prefix": instructions_prefix,
                ...
                print_task_progress(
                    1, 1, development_task["description"], "troubleshooting", "done", len(llm_solutions) + 1
                )
    ...
    def bug_report_generator(self, user_feedback, task_review_description):
        ...
        llm_response = bug_report_convo.send_message(
            "development/bug_report.prompt",
            {
                "user_feedback": user_feedback,
                "app_summary": self.project.project_description,
                "files": self.project.get_all_coded_files(),
        ...
        for missing_data_item in missing_data:
            ...
            questions_and_answers.append({"question": missing_data_item["question"], "answer": answer})
    ...
    def review_code_changes(self, llm_solutions):
        ...
        files = [
            file_dict
            for file_dict in self.project.get_all_coded_files()
            if any(
                os.path.normpath(file_dict["full_path"]).endswith(os.path.normpath(modified_file.lstrip(".")))
                for modified_file in self.modified_files
            )
        ]
        files_at_start_of_task = [
            file_dict
            for file_dict in self.files_at_start_of_task
            if any(
                os.path.normpath(file_dict["full_path"]).endswith(os.path.normpath(modified_file.lstrip(".")))
                for modified_file in self.modified_files
            )
        ...
        review = review_convo.send_message(
            "development/review_task.prompt",
            {
                "name": self.project.args["name"],
                "app_summary": self.project.project_description,
        ...
        llm_response = review_convo.send_message(
            "development/parse_task.prompt",
            {
                "os": platform.system(),
                "instructions_prefix": instructions_prefix,
    ...
    def test_code_changes(self, convo, task_steps=None, step_index=None):
        ...
        if test_type == "command_test":
            ...
            return run_command_until_success(
                convo, command["command"], timeout=command["timeout"], task_steps=task_steps, step_index=step_index
            )
    ...
    def implement_step(self, convo, step_index, type, description):
        ...
        llm_response = convo.send_message(
            "development/task/next_step.prompt",
            {
                "finished_steps": [],
    ...
    def get_alternative_solutions(
        self, development_task, user_feedback, previous_solutions, tried_alternative_solutions_to_current_issue
    ):
        convo = AgentConvo(self)
        response = convo.send_message(
            "development/get_alternative_solutions.prompt",
            {
                "name": self.project.args["name"],
                "app_type": self.project.args["app_type"],
                "app_summary": self.project.project_description,
        ...
        if isinstance(next_solution_to_try_index, str) and next_solution_to_try_index == NONE_OF_THESE:
            return self.get_alternative_solutions(
                development_task,
                user_feedback,
                previous_solutions,
                tried_alternative_solutions_to_current_issue + response["alternative_solutions"],
            )
    ...
    def ask_user_for_next_solution(self, alternative_solutions):
        ...
        string_for_buttons = (
            "/".join(solutions_indices_as_strings) + "/" + (NONE_OF_THESE[0].upper() + NONE_OF_THESE[1:])
        )

        description_of_solutions = "\n\n".join(
            [f"{index + 1}: {sol}" for index, sol in enumerate(alternative_solutions)]
        )
        print(string_for_buttons, type="button")
        next_solution_to_try_index = ask_user(
            self.project,
            "Choose which solution would you like GPT Pilot to try next?",
            require_some_input=False,
