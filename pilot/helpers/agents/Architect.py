...
from database.database import get_progress_steps, save_progress
...
class Architect(Agent):
    ...
    def get_architecture(self):
            ...
            if db_data:
                if isinstance(db_data, dict):
                    ...
                    self.project.system_dependencies = [
                        {"name": dep, "description": "", "test": "", "required_locally": False} for dep in db_data
                    ]
        ...
        warn_system_deps = [
            dep["name"] for dep in self.project.system_dependencies if dep["name"].lower() in WARN_SYSTEM_DEPS
        ]
        warn_package_deps = [
            dep["name"] for dep in self.project.package_dependencies if dep["name"].lower() in WARN_FRAMEWORKS
        ]
        ...
        if warn_system_deps:
            ...
            ask_user(
                self.project,
                "Press ENTER if you still want to proceed. If you'd like to modify the project description, close the app and start a new one.",
