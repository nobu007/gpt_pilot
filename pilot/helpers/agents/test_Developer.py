...
class TestDeveloper:
    def setup_method(self):
        ...
        self.project.set_root_path(
            os.path.abspath(
                os.path.join(os.path.dirname(os.path.abspath(__file__)), "../../../workspace/TestDeveloper")
            )
        )
    ...
    @patch("helpers.AgentConvo.save_development_step")
    @patch(
        "helpers.AgentConvo.create_gpt_chat_completion", return_value={"text": '{"tasks": [{"command": "ls -al"}]}'}
    )
    ...
    def test_test_code_changes_invalid_json(self, mock_requests_post, mock_save, mock_execute, monkeypatch):
        # Given
        convo = AgentConvo(self.developer)
        convo.save_branch = lambda branch_name=None: branch_name
        convo.load_branch = lambda function_uuid=None: function_uuid
        ...
        def generate_response(*args, **kwargs):
            ...
            gpt_response = json.dumps(
                {"type": types_in_response.pop(0), "command": {"command": "node server.js", "timeout": 3000}}
            )
