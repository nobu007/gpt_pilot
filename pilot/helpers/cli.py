...
import psutil
from const.code_execution import MAX_COMMAND_OUTPUT_LENGTH, MAX_COMMAND_RUN_TIME, MIN_COMMAND_RUN_TIME
from const.messages import AFFIRMATIVE_ANSWERS, NEGATIVE_ANSWERS
from database.database import save_command_run
from helpers.exceptions import CommandFinishedEarly, TokenLimitError, TooDeepRecursionError
...
def run_command(command, root_path, q_stdout, q_stderr) -> subprocess.Popen:
    ...
    if platform.system() == "Windows":  # Check the operating system
        process = subprocess.Popen(
            command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, cwd=root_path
        )
    else:
        process = subprocess.Popen(
            command,
...
def execute_command(
    ...
    try:
        ...
        terminate_process(
            process.pid
        )  # TODO: background_command - remove this is if we want to leave command running in background, look todo above
...
def check_if_command_successful(
    convo, command, cli_response, response, exit_code, additional_message=None, task_steps=None, step_index=None
):
...
def execute_command_and_check_cli_response(convo, command: dict, task_steps=None, step_index=None):
    ...
    cli_response, response, exit_code = execute_command(
        convo.agent.project, command["command"], timeout=command["timeout"], command_id=command_id
    )

    response = check_if_command_successful(
        convo, command["command"], cli_response, response, exit_code, task_steps=task_steps, step_index=step_index
    )
    ...
    if response != "DONE":
        ...
        while True:
            try:
                ...
                success = convo.agent.debugger.debug(
                    convo,
                    {
                        "command": command,
                        "timeout": timeout,
